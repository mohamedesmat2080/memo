...
namespace SilkroadSecurityAPI
{
    public class Security
    {
        ...
        public Security()
        {
            ...
            _mEncOpcodes = new List<ushort>();
            _mEncOpcodes.Add(0x2001);
            _mEncOpcodes.Add(0x6100);
            _mEncOpcodes.Add(0x6101);
            _mEncOpcodes.Add(0x6102);
            _mEncOpcodes.Add(0x6103);
            _mEncOpcodes.Add(0x6107);
        ...
        // Adds an encrypted msgId to the API. Any opcodes registered here will automatically
        // be encrypted as needed. Users should add the current Item Use msgId if they will be
        // mixing security modes.
        public void AddEncryptedOpcode(ushort msgId)
        {
            lock (m_class_lock)
            {
                if (_mEncOpcodes.Contains(msgId) == false)
                {
                    _mEncOpcodes.Add(msgId);
                }
            }
        }
            ...
            lock (m_class_lock)
            {
                ...
                while (length > 0)
                {
                    ...
                    while (_mRecvBuffer.Size > 0)
                    {
                        ...
                        if (max_copy_count > _mRecvBuffer.Size)
                        {
                            max_copy_count = _mRecvBuffer.Size;
                        }
                    ...
                    foreach (var buffer in incomingBuffersTmp)
                    {
                        ...
                        if (_mClientSecurity)
                        {
                            if (_mSecurityFlags.security_bytes == 1)
                            {
                                ...
                                {
                                    if (packet_encrypted || _mEncOpcodes.Contains(packet_opcode))
                                    {
                                        packet_size |= 0x8000;
