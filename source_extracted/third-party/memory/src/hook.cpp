...
void memoryAction(int trampoline_location,size_t Value,bool protect)
{
    if(protect)
    {
        if (!VirtualProtect((LPVOID)trampoline_location, Value, m_tmpProtect, &m_tmpProtect)) {
            perror("Failed to unprotect memory\n");
        }
    }
    else
    {
        if (!VirtualProtect((LPVOID)trampoline_location, Value,PAGE_READWRITE, &m_tmpProtect)) {
            perror("Failed to restore protection on memory");
        }
    }
}
void PatchMe(DWORD address, UINT8 value)
{
    DWORD oldProtect;
