...
void CICUser::Func_15(int param_1, float *param_2) {
    ...
    if (CGame::STA_FUN_004f9d00().field_c != '\0') {
        ...
        std::map<std::n_wstring, UINT32>::iterator foundedInfo;
        ...
        else
        {
            ...
            if(findCharNameColor != m_CharNameColor.end())
                this->UpdateNameColor(findCharNameColor->second);
            std::map<std::n_wstring, std::n_wstring>::iterator findCharName = m_CharName.find(this->GetName().c_str());
            if(findCharName != m_CharName.end())
            {
                wchar_t buffer[255];
                std::n_wstring mymsg;
                swprintf_s(buffer, sizeof(buffer), L"%s %s", findCharName->second.c_str(), this->GetName().c_str());
                mymsg.assign(buffer);
                this->fonttexture_playername.sub_8B3B60(&mymsg);
            }
            std::map<std::n_wstring, std::n_wstring>::iterator findCustomTitle = m_CustomTitle.find(this->GetName().c_str());
            if(findCustomTitle != m_CustomTitle.end())
            {
                ...
                mymsg.assign(buffer);
                if(this->m_hwanlevel == 0)
                    this->SetTitleName(5);
                this->fonttexture_title.sub_8B3B60(&mymsg);
            }
                std::map<std::n_wstring, int>::iterator findCustomTitleColor = m_CustomTitleColor.find(this->GetName().c_str());
            if(findCustomTitleColor != m_CustomTitleColor.end())
            {
                this->fonttexture_title.SetColor(findCustomTitleColor->second);
            }
            foundedInfo = m_VipIcons.find(this->GetName().c_str());
            if (foundedInfo != m_VipIcons.end())
            {
                std::map<int, IDirect3DBaseTexture9*>::iterator foundedPath = m_VipIconsData.find(foundedInfo->second);
                if(foundedPath!= m_VipIconsData.end())
                    g_RStateMgr.SetTextureForStage(0, foundedPath->second);
            }
        }

        float fVar5 = dataOut[0].x;
        if (trijobType != TRIJOB_NOJOB || foundedInfo != m_VipIcons.end()) {
        ...
        std::map<std::n_wstring, UINT32>::iterator foundedIcon = m_CharacterIcons.find(this->GetName().c_str());
        if (foundedIcon != m_CharacterIcons.end())
        {
            std::map<int, IDirect3DBaseTexture9*>::iterator foundedIconPath = m_IconsData.find(foundedIcon->second);
            if(foundedIconPath!= m_IconsData.end())
            {
                g_RStateMgr.SetTextureForStage(0, foundedIconPath->second);
            }
        }
        else
        {
        ...
        if(GetBeginnerState() != false || foundedIcon!= m_CharacterIcons.end())
        {
            int local_158 = 0;
            if (g_pDynamicVertexBuffer->IVBDynamic_Func_6(dataOut, 0x60, &local_158) != 0) {
                IDirect3DVertexBuffer9 *iVar5 = g_pDynamicVertexBuffer->IVBDynamic_Func_5();
...
void CICUser::ApplyTriJobNickName() {
    std::n_wstring wstr = g_pCGInterface->GetJobGradeName(GetJobType(),
                                                              this->m_refobjid,
                                                            this->m_jobrank);

    wchar_t buf[256];
    swprintf(buf, L"%s Lv.%d  ",wstr.c_str(),
             GetJobLevel());
    std::n_wstring title(L"");
    title.assign(buf);
    this->SetTitleName(5);
    this->fonttexture_title.sub_8B3B60(&title);
}