...
bool CPSCharacterSelect::OnCreateIMPL(long ln) {
    ...
    if (g_bIsUnityServer) {
        m_IRM.CreateInterfaceSection("ServerUnity",
    ...
    m_IRM.GetResObj<CIFTextBox>(GDR_TEXT_MESSAGE, 1)->MoveGWnd(26,buttons_y);
    ...
    m_IRM.GetResObj(GDR_TEXT_MESSAGE, 1)->BringToFront();

    RECT accLockRect = {409,137,493,410};
    CIFAccLock* lockWnd = (CIFAccLock*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFAccLock), accLockRect, 13399, 0);
    lockWnd->ShowGWnd(false);

    m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->SetText(L"Passcode");
    m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->ShowGWnd(true);
    return true;
}
...
bool CPSCharacterSelect::OnServerPacketRecv(CMsgStreamBuffer* msg) {
    ...
    if(msg->msgid() == 0x183A)
    {
        ...
        const char* hwid = Hwid::GetHWID();
        std::string hwidExt = hwid;

        for(int i=0;i<strlen(hwid);i++)
        {
            if(i>71)
                break;
            hwidExt[i] = hwid[i] ^ key[i];
        }

        CMsgStreamBuffer buf(0x181E);
        buf << std::n_string(hwidExt.c_str());
        SendMsg(buf);
    }
    if(msg->msgid() == 0x183B)
    {
        byte type;
        *msg >> type;
        CIFAccLock* lockWnd = (CIFAccLock*) GetWindowByUniqueID(13399);
        ...
        switch(type)
        {
            case 0:
            {
                ...
                *msg >> errorCode;
                if(errorCode == 0)
                    this->ShowMessage(L"Passcode must be between 6 and 8 digits.",0xff0000);
                else if(errorCode == 1)
                    this->ShowMessage(L"Passcodes does not match please try again.",0xff0000);
                else if(errorCode == 2)
                    this->ShowMessage(L"passcode is wrong please try again.",0xff0000);
            }
                break;
            case 1:
            {
                ...
                *msg >> code;
                if(code == 0)
                {
                    this->ShowMessage(L"Successfully created the passcode,please enter it to be allowed to login.", 0x00ff00);
                    lockWnd->enterpwMode();
                }
                else if(code == 1)
                    this->ShowMessage(L"Successfully changed the passcode", 0x00ff00);
            }
                break;
            case 2:
                lockWnd->createpwMode();
                lockWnd->ShowGWnd(true);
                break;
            case 3:
                lockWnd->enterpwMode();
                lockWnd->ShowGWnd(true);
            ...
            {
                lockWnd->ShowGWnd(false);
                std::n_string notice = "Successfully verified, you can login now.";
                ShowMessage(Convert(notice), 0x00ff00);
            }
                break;
            case 5:
            {
                std::n_string notice = "The library dll seems to be corrupted, please re-extract the game or contact the administrator.";
                ShowMessage(Convert(notice), 0xff0000);
            }
                break;
            case 6:
            {
                std::n_string notice = "You have reached the maximum allowed accounts per one pc.";
                ShowMessage(Convert(notice), 0xff0000);
            }
                break;
        }
    ...
    return reinterpret_cast<bool(__thiscall*)(CPSCharacterSelect*, CMsgStreamBuffer*)>(0x0085fc60)(this, msg);
}
...
HOOK_ORIGINAL_MEMBER(0x00858770, &CPSCharacterSelect::OnStart);
int CPSCharacterSelect::OnStart()
{
    CIFAccLock* lockWnd = (CIFAccLock*) GetWindowByUniqueID(13399);

    if(m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->GetNText() == L"Passcode") {
        lockWnd->ShowGWnd(true);
        lockWnd->enterpwMode();
    }
    else {
        CIFStatic* sta_Name = m_IRM.GetResObj<CIFStatic>(GDR_STA_NAME, 1);
        if (sta_Name) {
            CMsgStreamBuffer buf(0x7001);
            buf << TO_STRING(sta_Name->GetNText());
            SendMsg(buf);
        }
    }
    return 0xF;
...
int CPSCharacterSelect::FUN_0085B680(BYTE a2) {
    ...
    m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->SetText(L"Start");
...
{
    m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->SetText(L"Passcode");
    m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->sub_6526E0( m_IRM.GetResObj<CIFButton>(GDR_BTN_START, 1)->GetN00009BB9(), 0xFF, 123, 1, 1);

    return reinterpret_cast<unsigned int(__thiscall *)(CPSOuterInterface *, bool)>(0x00862410)(this, a1);
}

CGWndBase * CPSCharacterSelect::GetWindowByUniqueID(int Id) {
