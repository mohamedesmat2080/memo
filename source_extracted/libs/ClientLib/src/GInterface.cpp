...
void CGInterface::CreateFlorian0Event() {
    CGWnd *guide = GetAlarmManager()->GetGuide(GUIDE_CHEST);
}
...
void CGInterface::AfterOnCreate() {
    ...
    RECT toggleBtnRect = { CLIENT_SCREEN_WIDTH-12,CLIENT_SCREEN_HEIGHT/2 - 48,12,96 };
    (CIFMainMenu*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFMainMenu), toggleBtnRect, 13391, 0);

    RECT charRankRect = {409,137,535,527};
    (CIFCharRank*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFCharRank), charRankRect, 13392, 0);

    RECT uniqueLogRank = {409,137,568,381};
    (CIFUniqueLog*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFUniqueLog), uniqueLogRank, 13393, 0);

    RECT eventScheduleRect = {409,137,423,381};
    (CIFEventSchedule*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFEventSchedule), eventScheduleRect, 13394, 0);

    RECT itemChestRect = {409,137,709,441};
    (CIFItemChest*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFItemChest), itemChestRect, 13395, 0);

    RECT titleManagerRect = {409,137,571,473};
    (CIFTitleManager*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFTitleManager), titleManagerRect, 13396, 0);

    RECT customsMarketRect = {409,137,810,363};
    (CIFCustomsMarket*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFCustomsMarket), customsMarketRect, 13397, 0);

    RECT dailyRewardRect = {409,137,810,600};
    (CIFDailyReward*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFDailyReward), dailyRewardRect, 13398, 0);

    RECT dpsRect = {0,100,300,240};
    (CIFDps*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFDps), dpsRect, 13400, 0);

    RECT partyinfoRect = {409,137,709,352};
    (CIFPartyInfo*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFPartyInfo), partyinfoRect, 13401, 0);

//    RECT wheelRect = {409,137,400,400};
//    (CIFWheelFortune*)CGWnd::CreateInstance(this, GFX_RUNTIME_CLASS(CIFWheelFortune), wheelRect, 13403, 0);
...
int CGInterface::OnKeyDown(int keycode, int a3, char a4)
{
    switch (keycode) {
        case 70:
            reinterpret_cast<void(__thiscall*)(CGInterface*, bool)>(0x00799020)(this, true);
            break;
        case 27:
        ...
        case 116:
            for(int i = 13392;i<= 13398;i++)
            {
                ...
                if(i == 13393 && Wnd)
                {
                    CIFUniqueLog* uniqueWnd = (CIFUniqueLog *) Wnd;
        ...
        case 117:
            for(int i = 13392;i<= 13398;i++)
            {
                ...
                if(i == 13394 && Wnd)
                {
                    CIFEventSchedule* eventWnd = (CIFEventSchedule *) Wnd;
        ...
        case 118:
            for(int i = 13392;i<= 13398;i++)
            {
                ...
                if(i == 13395 && Wnd)
                {
                    CIFItemChest* chestWnd = (CIFItemChest *) Wnd;
                    if(chestWnd)
                    {
...
CGWndBase* CGInterface::GetWindowByUniqueID(int UniqueID)
{
...
std::n_wstring CGInterface::GetJobGradeName(TRIJOB_TYPE byJobType, int byKindred, BYTE btHWanLvl) const {
    if(btHWanLvl == 0)
        return TSM_GETTEXTPTR(L"UIIT_STT_NONE");

    std::n_wstring wstrFormat;
    wstrFormat = L"UIIT_STT_CLASS_EU_THIEF_%d";
    if(byKindred < 2000)
        wstrFormat = L"UIIT_STT_CLASS_THIEF_%d";

    if(byJobType == TRIJOB_HUNTER) {
        wstrFormat = L"UIIT_STT_CLASS_EU_HUNTER_%d";
        if(byKindred < 2000)
            wstrFormat = L"UIIT_STT_CLASS_HUNTER_%d";
    }
    else if(byJobType == TRIJOB_TRADER) {
        wstrFormat = L"UIIT_STT_CLASS_EU_MERCHANT_%d";
        if(byKindred < 2000)
            wstrFormat = L"UIIT_STT_CLASS_MERCHANT_%d";
    }
    wchar_t buf[256];
    swprintf_s(buf, sizeof(buf), wstrFormat.c_str(), btHWanLvl);
    wstrFormat.clear();

    return TSM_GETTEXTPTR(buf);
}