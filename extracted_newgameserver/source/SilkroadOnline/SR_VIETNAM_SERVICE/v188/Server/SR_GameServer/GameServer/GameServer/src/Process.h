...
{
    unsigned int valueLength = strlen(Value) + 1;
...
static std::string ReadMemoryString(DWORD DestAddress)
{
    ...
    unsigned __int32 charPtr;
    if (ReadMemoryValue<unsigned __int32>(DestAddress, charPtr))
...
std::string extension_removal(std::string const& name_of_file)
{
    size_t index_of_period = name_of_file.find_last_of('.');
    std::string file_without_extension = name_of_file.substr(0, index_of_period);
...
static std::string GetExecutablePath()
{
    ...
    std::string(filename, GetModuleFileNameA(hModule, filename, MAX_PATH));
    return extension_removal(clear_slash(filename));
}