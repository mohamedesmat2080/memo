...
GFX_IMPLEMENT_DYNCREATE(CIFItemUnlocker, CIFMainFrame)
GFX_BEGIN_MESSAGE_MAP(CIFItemUnlocker, CIFMainFrame)
                    ONG_COMMAND(GDR_LOCK_BUTTON, &CIFItemUnlocker::On_BtnClick_1)
                    ONG_COMMAND(GDR_CANCEL_BUTTON, &CIFItemUnlocker::On_BtnClick_2)
                    ONG_COMMAND(GDR_CERT_BUTTON, &CIFItemUnlocker::OnBtnCert)
GFX_END_MESSAGE_MAP()

CIFItemUnlocker::CIFItemUnlocker(void) {
    ok = 0;
    cancel = 0;
    write = 0;
    ItemSlotID = -1;
...
bool CIFItemUnlocker::OnCreate(long ln) {
    //	printf("> " __FUNCTION__ "(%d)\n", ln);

    // Populate inherited members
    CIFMainFrame::OnCreate(ln);
    // TB_Func_13("interface\\frame\\mframe_wnd_", 0, 0);
    m_IRM.LoadFromFile("clientlibrary\\resinfo\\ifitemunlocker.txt");
    ...
    m_IRM.GetResObj(14, 1)->SetText(L"Unlock Item");
    ...
    sz.size.width = 32;
    sz.size.height = 32;
    ItemSlot = m_IRM.GetResObj<CIFItemUnlockSlot>(500, 1);
...
void CIFItemUnlocker::OnUpdate() {
}

void CIFItemUnlocker::SetItemInfo(int ItemSlotIDs, unsigned short ItemTypeIDs) {
    ItemSlotID = ItemSlotIDs;
    ItemTypeID = ItemTypeIDs;
}
void CIFItemUnlocker::UpdateMenuSize() {
...
void CIFItemUnlocker::EnableStateLockButton()
{
    this->m_IRM.GetResObj<CIFButton>(GDR_LOCK_BUTTON, 1)->SetEnabledState(true);
}
void CIFItemUnlocker::On_BtnClick_1() {
    if(Pass1->GetCurrentText().size() > 5)
    {
        ...
        if (ItemSlotID >= 0 && ItemSlotID <= 95) {
            if(this->ItemSlot->m_pMySlot->m_pSlot->ItemInfo != NULL)
            {
                ...
                if (item->GetItemData()->m_typeId.m_type_id_value == 0xD6ED) {
...
void CIFItemUnlocker::On_BtnClick_2() {
    Clear();
    this->OnCloseWnd();

}
undefined1 CIFItemUnlocker::OnCloseWnd(){
    Clear();

    return CIFWnd::OnCloseWnd();
}

void CIFItemUnlocker::OnBtnCert()
{
    if(this->ItemSlot->m_pMySlot->m_pSlot->ItemInfo != NULL)
    {
        CMsgStreamBuffer buf(0x169A);
        buf << byte(22);
        buf << byte(1); // unlock TYPE
        SendMsg(buf);
        this->m_IRM.GetResObj<CIFButton>(GDR_CERT_BUTTON, 1)->SetEnabledState(false);
    }
}

void CIFItemUnlocker::StartRemainingTime()
{
...
void CIFItemUnlocker::OnTimer(int timerId) {
