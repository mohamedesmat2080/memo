...
GFX_BEGIN_MESSAGE_MAP(CIFChatViewer, CIFWnd)
                    ...
                    ONG_COMMAND(GDR_EMOJIES, &CIFChatViewer::OnEmojiButtonClick)

GFX_END_MESSAGE_MAP()

void CIFChatViewer::OnEmojiButtonClick()
{
    if(g_pCGInterface->m_IRM.GetResObj<CIFCustomEmojiList>(1360, 1)->IsVisible())
    {
        g_pCGInterface->m_IRM.GetResObj<CIFCustomEmojiList>(1360, 1)->ShowGWnd(false);
    }
    else
    {
        g_pCGInterface->m_IRM.GetResObj<CIFCustomEmojiList>(1360, 1)->ShowGWnd(true);
...
bool CIFChatViewer::OnCreate(long ln) {
    m_IRM.LoadFromFile("clientlibrary\\resinfo\\ifchatviewer.txt");

    m_IRM.CreateInterfaceSection("Create", this);

    m_pWhisperList = m_IRM.GetResObj<CIFWhisperList>(GDR_WHISPERLIST, 1);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        m_ChatLists[i]->SetBottomAligned(true);
        m_ChatLists[i]->SetIndentAfterBreak(true);
        m_ChatLists[i]->SetRespondToMouseMove(true);
    ...
    m_Button_ChatTabHide->SetTooltip(TSM_GETTEXTPTR(L"UIIT_STT_CHATING_VIEW_AUTO_OFF"));
    m_Button_ChatTabHide->SetStyleThingy(TOOLTIP);

    m_Button_WhisperList->SetTooltip(TSM_GETTEXTPTR(L"UIIT_CTL_WHISPER_LIST"));
    m_Button_WhisperList->SetStyleThingy(TOOLTIP);

    m_Button_ChatSize->SetTooltip(TSM_GETTEXTPTR(L"UIIT_STT_TOGGLE_CHATING_WINDOW_SETING"));
    ...
    UpdateChatSize();
    std::n_wstring EmojiList = L"Emoji list";
    this->m_IRM.GetResObj(500, 1)->SetStyleThingy(TOOLTIP);
    this->m_IRM.GetResObj(500, 1)->SetTooltip(EmojiList);
    this->m_IRM.GetResObj(500, 1)->ShowGWnd(false);
    if(m_Settings->ENABLE_EMOJI_SYSTEM)
    {
        this->m_IRM.GetResObj(500, 1)->ShowGWnd(true);
    }
    //initializeEmojis();
...
void CIFChatViewer::OnUpdate()
{
    reinterpret_cast<void(__thiscall *)(CIFChatViewer *)>(0x006528a0)(this);
    wnd_pos x = this->m_IRM.GetResObj<CIFButton>(41, 1)->GetPos();

    this->m_IRM.GetResObj<CIFButton>(500, 1)->MoveGWnd(x.x + 390, x.y);
...
int CIFChatViewer::OnChar(UINT nChar, UINT a2, UINT a3) {
    ...
    if (!m_bChattingAllowed) {
        ...
        g_pCGInterface->WriteSystemMessage(SYSLOG_NONE, buffer);
        return 0;
    }
        ...
        case VK_RETURN: {
            ...
            switch (input_text[0]) {
                ...
                case '&':
                    ...
                    break;
                default:
                    if (g_pMyPlayerObj->IsGameMaster()) {
            ...
            if (!sub_4F9C50()) {
                ...
                if (N00009C5E == CHAT_Union && g_pMyPlayerObj && g_CCharacterDependentData.IsGuildInAnAlliance()) {
                    wchar_t buffer[256];
                    swprintf_s(buffer, L"-%s): ", g_pMyPlayerObj->GetGuildName().c_str());
...
#if 0
                            pReq << g_global->m_LinkedGlobalSlot;
#endif
                            break;
...
void CIFChatViewer::OnWhisperList() {
    m_pWhisperList->ShowGWnd(!m_pWhisperList->IsVisible());
}
...
void CIFChatViewer::SetWhisperTarget(std::n_wstring &targetName) {
    ...
    if (g_pMyPlayerObj->GetCharName() == targetName) {
...
void CIFChatViewer::SetLastWhisperedWith(std::n_wstring &wstrName) {
    if (g_pMyPlayerObj == NULL)
        return;

    if (g_pMyPlayerObj->GetCharName() == wstrName)
        return;

    N00009C82 = wstrName;
}

void CIFChatViewer::AddWhisperTargetToList(std::n_wstring &wstrName) {
    if (g_pMyPlayerObj == NULL)
        return;
    ...
    m_pWhisperList->AddToList(wstrName);
    SetLastWhisperedWith(wstrName);
}
