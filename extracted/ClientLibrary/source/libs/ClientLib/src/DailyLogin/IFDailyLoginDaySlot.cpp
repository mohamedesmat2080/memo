...
GFX_IMPLEMENT_DYNCREATE(CIFDailyLoginDaySlot, CIFWnd)
GFX_BEGIN_MESSAGE_MAP(CIFDailyLoginDaySlot, CIFWnd)

GFX_END_MESSAGE_MAP()
CIFDailyLoginDaySlot::CIFDailyLoginDaySlot(void)
{
...
bool CIFDailyLoginDaySlot::OnCreate(long ln)
{
...
void CIFDailyLoginDaySlot::OnUpdate() {

}
void CIFDailyLoginDaySlot::ShowOK()
{
    m_IRM.GetResObj(16, 1)->ShowGWnd(true); // Tek sayı günlerde gizle

}
void CIFDailyLoginDaySlot::ClearOK()
{
    m_IRM.GetResObj(16, 1)->ShowGWnd(false); // Tek sayı günlerde gizle

}
void CIFDailyLoginDaySlot::UpdateCalenderVisual(int day, const char* month, int year, const char* dayOfWeek)
{
    if(day < 31)
    {
        if (day < 10)
