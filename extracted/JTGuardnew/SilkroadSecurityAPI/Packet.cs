...
namespace SilkroadSecurityAPI
{
    public class Packet
    {
        ...
        public void WriteWString(string value, int codePage)
        {
            if (m_isReadOnly)
            {
                throw new Exception("Cannot Write to a locked Packet.");
            ...
            // Veriyi istenilen kod sayfasına dönüştürme
            byte[] codepageBytes = Encoding.GetEncoding(codePage).GetBytes(value);

            // UTF-8'e dönüştürme
            string utf8Value = Encoding.UTF8.GetString(codepageBytes);

            // Boyutun yazılması
            m_writer.Write((ushort)utf8Value.Length);

            // Verinin yazılması
            m_writer.Write(Encoding.Default.GetBytes(utf8Value));
        }
